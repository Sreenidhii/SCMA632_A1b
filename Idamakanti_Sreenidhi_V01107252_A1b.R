...
data <- read.csv("IPL_ball_by_ball_updated till 2024.csv")
# Load required libraries
library(dplyr)
library(readr)
library(stringr)
...
# Filter the runs scored by JC Buttler
JC_Buttler_runs <- runs %>% filter(Striker == "JC Buttler") %>% pull(runs_scored)
...
# Fit the distribution to JC Buttler's runs scored and get the best distribution
best_distribution <- get_best_distribution(JC_Buttler_runs)
